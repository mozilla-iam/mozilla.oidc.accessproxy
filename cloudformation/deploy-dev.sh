...
aws cloudformation create-stack --stack-name ${STACK_NAME}-dev --template-body file://cloudformation/us-west-2.yml
--capabilities CAPABILITY_NAMED_IAM --parameters  ParameterKey=SSHKeyName,ParameterValue=${SSH_KEYS} ParameterKey=EnvType,ParameterValue=dev
...
aws kms create-grant --key-id $credstash_key_id --grantee-principal $role_arn --operations "Decrypt" --constraints $constraints --name ${STACK_NAME}
